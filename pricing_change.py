...
def main(list_orders):
    for order in list_orders:
        ...
        st.code(f'{order}' + ' Updated')
        
def main_regenarte_single(list_orders):
    for order in list_orders:
        order_number = order
        order_data = all_admin_orders_accounting_page(order_number)
...
if list_orders is not None:
        df = pd.read_excel(list_orders,engine='openpyxl')
        df['Invoice'] = df['Invoice'].astype('str') 
        count_invoices = df.shape
        st.write(f'{count_invoices[0]} Invoices to Update')

        with col2:
            selection = st.radio('Update Flat Fee or Regenerate Invoice',options=['Update Fee','Regenerate Single Invoice','Regenerate Invoice B'],)
            ...
            elif selection == 'Regenerate Single Invoice':            
                    st.caption('Click Button below to process')
                    submit_to_regenerate_single = st.button('Regenerate Single Invoice')
                    if submit_to_regenerate_single:
                        main_regenarte_single(df["Invoice"])
                            
            else:
                    st.caption('Click Button below to process')
                    submit_to_regenerate = st.button('Regenerate Invoice B')
                    if submit_to_regenerate:
